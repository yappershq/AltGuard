...
/// <summary>
/// Loads a shared SteamID bypass list from configs/&lt;file&gt; that BOTH AltGuard and AntiVpnGuard
/// read, so one edit exempts a player from connection-gating in both plugins.
    ...
    public static HashSet<string> Load(string sharpPath, string fileName, ILogger logger)
    {
        ...
        try
        {
            ...
            if (parsed?.SteamIds is { Count: > 0 })
            {
                ...
                logger.LogInformation("[AltGuard] Loaded {Count} shared-bypass SteamIDs from {File}", result.Count, fileName);
            }
        }
        catch (Exception e)
        {
            logger.LogWarning(e, "[AltGuard] Failed to read shared bypass file '{Path}'", path);
        }
        return result;
    }
}
